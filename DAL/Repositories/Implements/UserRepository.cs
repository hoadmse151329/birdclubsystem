    ...
    public class UserRepository : RepositoryBase<User>, IUserRepository
    {
        ...
        public async Task<User?> GetByEmail(string email)
        {
            if(await _context.Members.AsNoTracking().SingleOrDefaultAsync(mem => mem.Email == email) != null)
                return await _context.Users.AsNoTrackingWithIdentityResolution().Include(usr => usr.MemberDetails).SingleOrDefaultAsync(usr => usr.MemberDetails.Email == email);
            return null;
        }

        public async Task<User?> GetByIdNoTracking(int id)
        {
            return await _context.Users.AsNoTrackingWithIdentityResolution().Include(usr => usr.MemberDetails).SingleOrDefaultAsync(usr => usr.UserId == id);
        }

        public async Task<User?> GetByLogin(string userName, string passWord)
        {
            return await _context.Users.AsNoTrackingWithIdentityResolution().Include(usr => usr.MemberDetails).SingleOrDefaultAsync(usr => usr.UserName == userName && usr.Password == passWord);
        }
        ...
        {
            var usr = await _context.Users.AsNoTrackingWithIdentityResolution().Include(usr => usr.MemberDetails).SingleOrDefaultAsync(usr => usr.UserId == id);
            if (usr != null)
            {
                return usr.MemberId;
            }
